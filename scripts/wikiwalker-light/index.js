		...
		articles.forEach(function(item) {
			...
			wiki.page(name).then(function(page) {
				page.content().then(function(text) {
					...
					var dbEntry = { id: slugg(name), title: name, text: text, length: text.length, source: item, date: new Date() };
					...
					(db.collection('diseases')).update(duplicateTest, { $setOnInsert: dbEntry }, { upsert: true }, function (err, result) { //Insert the document
						assert.equal(null, err);
